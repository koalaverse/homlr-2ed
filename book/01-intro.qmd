...
nearly all domains.^[There's no shortage of definitions for the term machine learning. For the purposes of this book, we can think of it as a blended field with a focus on using algorithms to help *learn from data*.] Some example applications of machine learning in practice include:
...
and type of supervision needed during training.  The two main groups of learners this book focuses on
are: 
...
A *predictive model*\index{predictive model} is used for tasks that involve the
prediction of a given output (or target variable) using a set of relevant input variables (often referred to as features or predictors). Or, as stated by @apm [p. 2], predictive modeling is "...the process of developing
...
next 6 weeks.
* Using varous home attributes to predict the sales price.
...
($X$) to predict a well-defined target ($Y$). Think about the hostpital readmission example. Predicting the liklihood of readmittance is not specific enough and will make pulling togethe relevant data a challenge, so we need to think carefully aabout how we define the features and response for modleing. Defining the target as *whether or not a patient was readmitted within 30 days after release* is something that can easily be measured, assuming it's relevant to the stakeholders.

The scenarios listed above are examples of _supervised learning_\index{supervised learning}.
...
_regression_\index{regression} or _classification_\index{classification}, which we'll
discuss next.
