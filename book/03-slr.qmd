...
where $x_i$ and $y_i$ are the individual sample points and $\bar{x}$ and $\bar{y}$ are the sample mean. There are alternative methods that allow us to loosen some assumptions such as assuming a linear relationship; however, in the next section we'll see how simple linear regression fully characterizes this Pearson's correlation.
